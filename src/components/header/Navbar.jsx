...
export default function Navbar() {
    ...
    return (
        <header className="fixed top-0 w-full z-50 bg-[var(--color-bg)]">
            <nav className=" w-93/100 max-w-[800px] mx-auto pt-4 flex justify-between items-center lg:max-w-[1440px] lg:pt-0">
                ...
                <ul ref={navLinksRef} className={` bg-[var(--color-bg)] invisible fixed w-full min-h-full top-0 left-0 transform translate-x-[-100%] transition duration-500 lg:visible lg:static lg:w-1/2 min-h-auto lg:transform-none lg:translate-x-[0] lg:transition-none ${isActive ? "visible transform-none translate-none": "invisible"}`}>
                    <div className=" w-1/2 text-center mx-auto my-30 lg:w-full lg:text-left lg:mx-0 lg:my-0 lg:flex lg:justify-evenly">
                         <li className={`${activeSection === 'services' ? 'border-b-2 border-[var(--color-g1)]' : ''} text-[var(--color-dim)] hover:text-[var(--color-cream)] uppercase tracking-widest text-sm p-8 cursor-pointer`}><a href="#services">Services</a></li>
                        <li className={`${activeSection === 'work' ? 'border-b-2 border-[var(--color-g1)]' : ''} text-[var(--color-dim)] hover:text-[var(--color-cream)] uppercase tracking-widest text-sm p-8 cursor-pointer`}><a href="#work">Work</a></li>
                        <li className={`${activeSection === 'about' ? 'border-b-2 border-[var(--color-g1)]' : ''} text-[var(--color-dim)] hover:text-[var(--color-cream)] uppercase tracking-widest text-sm p-8 cursor-pointer`}><a href="#about">About</a></li>
                        <li className={`${activeSection === 'technologies' ? 'border-b-2 border-[var(--color-g1)]' : ''} text-[var(--color-dim)] hover:text-[var(--color-cream)] uppercase tracking-widest text-sm p-8 cursor-pointer`}><a href="#technologies">Technologies</a></li>
                        <li className={`${activeSection === 'contact' ? 'border-b-2 border-[var(--color-g1)]' : ''} text-[var(--color-muted)] hover:text-[var(--color-cream)] uppercase tracking-widest text-sm p-8 cursor-pointer`}><a href="#contact">Contact</a></li>
                        <li className="grad-bg text-[var(--color-bg)] font-semibold py-4 px-6 rounded-md my-4 mx-4 cursor-pointer hover:opacity-90 whitespace-nowrap"><a href="#contact">Hire Me</a></li>
                    </div>
                </ul>
            </nav>
        </header>
